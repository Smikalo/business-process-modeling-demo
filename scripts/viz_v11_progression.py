"""V1 → V11 progression visualization.

Summarizes the entire model evolution, validation + test SIMSCORE / WAPE
/ Monthly-WAPE / aggregate bias, plus a residual heatmap for V11.
"""
...
MODELS = [
    ...
    ("V10 LAD",              "v10_lad",     "#666666"),
    ("V11 (champion)",       "v11_final",   "#000000"),
]
...
def main() -> int:
    ...
    summ.to_csv(OUT / "v11_progression_summary.csv", index=False)
    ...
    fig.suptitle("V1 → V11 model evolution: SIMSCORE / WAPE / Monthly-WAPE / Bias",
                 fontsize=14, weight="bold")
    fig.tight_layout()
    fig.savefig(OUT / "plot_v11_progression.png", dpi=140, bbox_inches="tight")
    print(f"wrote {OUT/'plot_v11_progression.png'}")

    df = _load("v11_final", "test")
    if df is not None:
        ...
        ax.set_title("V11 channel × month residual heatmap (test set)\n"
                     "blue = V11 over-predicts | red = V11 under-predicts")
        fig2.colorbar(im, ax=ax, label="(actual − pred) / (|actual|+1)")
        fig2.tight_layout()
        fig2.savefig(OUT / "plot_v11_residual_heatmap.png", dpi=140,
                     bbox_inches="tight")
        print(f"wrote {OUT/'plot_v11_residual_heatmap.png'}")
    return 0
