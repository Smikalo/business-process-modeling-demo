"""V8 — V1 -> V8 progression visualisation + V8 residual heatmap.

Mirrors viz_v78_progression but adds V8 as the new champion and refreshes
...
def _aggregate_residual_table(tag: str = "v8_lad") -> pd.DataFrame:
...
def main() -> int:
    ...
    summary[["label", "tag", "WAPE", "SIMSCORE", "Agg_Bias_pct",
        ...
        OUT / "v8_progression_summary.csv", index=False
    )
    ...
    ax1.set_title("Test SIMSCORE - V4 → V8 (lower is better)")
    ...
    ax2.set_title("Test WAPE - V4 → V8 (lower is better)")
    ...
    fig.suptitle(
        "Demand forecasting - V1 → V8 progression on the held-out test set\n"
        "V8 = V7.8 LAD pool + 2 NEW within-month-aware bases (v8, v8_recent) "
        "+ 23 daily-shipment-derived features + school calendar",
        fontsize=13, weight="bold", y=1.00,
    )

    path = OUT / "plot_v8_progression.png"
    fig.savefig(path, dpi=140, bbox_inches="tight")
    print(f"wrote {path}")

    df = _aggregate_residual_table("v8_lad")
    pivot = (
    ...
    ax.set_title(
        "V8 residual heatmap - aggregate bias % by Канал × month-of-year\n"
    ...
    fig2.tight_layout()
    out_path = OUT / "plot_v8_residual_heatmap.png"
