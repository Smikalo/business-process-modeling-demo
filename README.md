...
CV-picked from 30 candidates by **recency-weighted OOF** (fold weights 0.2 / 0.3 / 0.5, favouring the most recent fold); gap **+0.0099**, below the 0.02 ceiling.  Result: **test SIMSCORE 0.4875 → 0.4827** (−1.0 %), **WAPE 0.4255 → 0.4230** (−0.6 %), **A-class bias −12 % → −2 %**, **portfolio-level WAPE 0.0697 → 0.0674** — a new all-time low.  See `docs/reports/v77_final_report.md`.
...
See `docs/adr/adr-003-external-signals.md` for the full decision record and `output/decision_gate.md` for the per-loader verdict table.
...
Artefacts: `output/model_v7.joblib`, `output/preds_v7_{val,test,lower,upper,stacked}_*.csv`, `output/v7_metrics.csv`, `output/v7_rolling_cv.{json,md}`, `output/cost_scorecard_final.{md,json}`. Full ADR: `docs/adr/adr-005-v7.md`. Executive report: `docs/reports/v7_final_report.md`.
...
Artefacts: `output/model_v7_{rec95,ch_im,ch_nkp,ch_rs,ch_sk}.joblib`, `output/preds_v71_{val,test}.csv`, `output/cost_scorecard_v71_channels.{md,json}`, `output/plot_v71_{dashboard,recency_sweep,channel_blend,stability}.png`. Full ADR: `docs/adr/adr-006-v71.md`. Executive report: `docs/reports/v71_final_report.md`.
...
Artefacts: `output/preds_v9_lad_{val,test}.csv` (production), `output/preds_v9_{val,test}.csv` and `output/preds_v9_recent_{val,test}.csv` (sales-leading bases), `output/preds_v9_weekly_{val,test}.csv` (weekly Tweedie base), `output/v9/lad_champion.json`, `output/v9/lad_cv.csv`, `output/feature_importance_v9.csv`, `output/feature_importance_v9_weekly.csv`, `output/model_v9.joblib`, `output/model_v9_recent.joblib`, `output/plot_v9_dashboard.png`, `output/plot_v9_progression.png`, `output/plot_v9_sales_features.png`, `output/plot_v9_multi_resolution.png`, `output/plot_v9_residual_heatmap.png`, `output/abt_v9_cached.parquet`, `output/v9_progression_summary.csv`. Executive report: `docs/reports/v9_final_report.md`.
...
Artefacts: `output/preds_v8_lad_{val,test}.csv` (production), `output/preds_v8_{val,test}.csv` and `output/preds_v8_recent_{val,test}.csv` (bases), `output/v8/lad_champion.json`, `output/v8/lad_cv.csv`, `output/feature_importance_v8.csv`, `output/plot_v8_dashboard.png`, `output/plot_v8_progression.png`, `output/plot_v8_within_month_features.png`, `output/plot_v8_residual_heatmap.png`, `output/abt_v8_cached.parquet`, `output/v8_progression_summary.csv`. Executive report: `docs/reports/v8_final_report.md`.
...
Artefacts: `output/preds_v78_{val,test}.csv`, `output/v78/lad_champion.json`, `output/v78/lad_cv.csv`, `output/plot_v78_{dashboard,progression,residual_heatmap}.png`, `output/plot_models_timeline.png`, `output/v78_progression_summary.csv`.  Executive report: `docs/reports/v78_final_report.md`.
...
See `docs/v4-creative-approaches.md` for full technical writeup and `docs/adr/adr-002-ensemble-architecture.md` for the architecture decision.
...
See [`STRUCTURE.md`](STRUCTURE.md) for the full layout map. High-level overview:
...
├── requirements.txt
│
├── data/                         ← raw client data (gitignored .xlsx/.csv/.txt)
├── src/                          ← core importable library (model/feature/loader code)
├── scripts/                      ← runnable CLI scripts (~120, build/train/audit/viz)
...
│   ├── retrospectives/           ← campaign retros (V12, V12.1, V12.2, V13.1, V14)
...
│   ├── v14_*                     ← V14 GlobalNN training + Kaggle pipeline state
...
|--------|---------|------------------|
| `pipelines/run_pipeline.py` | Original V1 pipeline (ingest → V1 → recommendations + plots) | ~30 min |
| `pipelines/run_v4_experiments.py` | Train all V4 base models (V3, PerChannel, LogTarget, Reconciled, baselines) | ~8 min |
| `pipelines/run_v4_round2.py` | Post-hoc calibration + GBDT meta-learner experiments | ~1 min |
| `pipelines/run_v4_final.py` | Production V4 ensemble | ~3 min |
...
| `docs/guides/v11_chronos_colab_guide.md` | **Step-by-step Colab guide** for the V11 Chronos run | (read-only) |
...
PYTHONPATH=. python3 pipelines/run_v4_final.py

# Or: inspect every model side-by-side
PYTHONPATH=. python3 pipelines/run_v4_experiments.py
```
...
- **LightGBM on CPU** — no GPU, no cloud, zero cost. Trains 2.8M rows in ~40s; V4 full ensemble in ~3 min. See `docs/adr/adr-001`.
...
- **Convex-blend ensemble** (V4): 3–4 weighted models beat learned GBDT meta-learner under limited validation data. See `docs/adr/adr-002`.
...
See `docs/guides/limitations-and-next-steps.md` for the full roadmap.
